...
import { removeAllFileEntries } from "./io.js?v=2.1.2.0";
...
export function initialize(adapter, element, elementId) {
    ...
    _instances[elementId] = new FileInputInfo(adapter, element, elementId);
...
export function reset(element, elementId) {
    ...
    if (element) {
        ...
        var fileInputInfo = _instances[elementId];

        if (fileInputInfo) {
            fileInputInfo.adapter.invokeMethodAsync('NotifyChange', []).then(null, function (err) {
                throw new Error(err);
            });
        }
    }
